...
namespace Unity.InferenceEngine.ONNX.Editor
{
    ...
    class ONNXModelConverter
    {
        ...
        public Model Convert()
        {
            ...
            Model model = ConvertOnnxModel(onnxModel);
            if (model == null || model.layers == null)
            {
                var extra = Warnings.Count > 0 ? Warnings.Last().Message : "unknown importer failure";
                throw new OnnxImportException(
                    "ONNX conversion returned null. Unity Inference Engine 2.3 needs a classic ONNX " +
                    $"(opset 7–15, TorchScript exporter — not PyTorch dynamo/opset 18). {extra}");
            }
