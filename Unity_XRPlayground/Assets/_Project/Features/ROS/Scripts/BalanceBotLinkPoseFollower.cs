...
{
    /// <summary>
    /// Applies /xr/balance_bot/robot_state link poses onto the Balance Bot hierarchy.
    /// </summary>
    public sealed class BalanceBotLinkPoseFollower : MonoBehaviour
    {
        public RosTcpClient client;
        public BalanceBotLinkMap linkMap;
        public Transform envAnchor;
        public string robotStateTopic = RosTopics.BalanceBotRobotState;
        ...
        {
            if (linkMap == null)
                linkMap = GetComponent<BalanceBotLinkMap>();
            if (envAnchor == null)
                envAnchor = transform;
        }
        ...
        public void Apply(RobotStateData state)
        {
            ...
            {
                _loggedFirst = true;
                Debug.Log($"BalanceBotLinkPoseFollower: applied {applied}/{state.links.Length} links.", this);
            }
        }
    }
}
