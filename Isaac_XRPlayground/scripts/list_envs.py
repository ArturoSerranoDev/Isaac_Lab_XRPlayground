...
All the environments are registered in the `XRPlayground` extension. They start
...
with contextlib.suppress(ImportError):
    import XRPlayground.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~XRPlayground.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from XRPlayground.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `XRPlayground` extension."""
    ...
    if args_cli.show_presets:
        from XRPlayground.utils.preset_cli import enumerate_task_presets
